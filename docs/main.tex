...
las dificultades encontradas y las alternativas descartadas. La
Sección~\ref{sec:conclusiones} presenta las conclusiones del trabajo, y la
Sección~\ref{sec:limitaciones} resume las limitaciones identificadas en el
estado actual del compilador junto con recomendaciones priorizadas de trabajo
futuro.
...
(padre-antes-hijo, orden de declaraci\'{o}n). La llamada virtual consiste en cargar el puntero al vtable, indexar por el slot del m\'{e}todo luego llamarlo indirectamente.
...
cualquier ciclo que el programa construya. El dise\~{n}o adoptado es, por ello,
h\'{i}brido: conteo de referencias como camino r\'{a}pido para el caso ac\'{i}clico,
combinado con un recolector de marcado y barrido reservado para romper los
ciclos que el conteo de referencias nunca libera por s\'{i} solo. Este punto del
espacio de dise\~{n}o -- combinar ambas t\'{e}cnicas en lugar de optar por un
extremo \'{u}nico -- corresponde a la categor\'{i}a que Bacon, Cheng y Rajan
identifican como la m\'{a}s eficaz en la pr\'{a}ctica dentro de su taxonom\'{i}a
unificada de algoritmos de recolecci\'{o}n de basura~\cite{baconrajan2004}, y es,
en sustancia, la misma estrategia que adopta CPython desde sus primeras
versiones: conteo de referencias para el camino com\'{u}n, con un recolector
generacional adicional reservado para romper ciclos~\cite{baconrajan2001}.
...
\begin{itemize}
    \item \textbf{Conteo de referencias}: cada asignaci\'{o}n incrementa el contador
          ...
          Esta mitad del modelo \textbf{est\'{a} implementada}: \texttt{hulk-rt}
          expone \texttt{hulk\_rt\_alloc}, \texttt{hulk\_rt\_retain} y
          \texttt{hulk\_rt\_release}, y \texttt{hulk-codegen} los invoca
          sistem\'{a}ticamente en la construcci\'{o}n de objetos, la asignaci\'{o}n
          a miembros y, en particular, al cerrar cada \'{a}mbito l\'{e}xico
          (\texttt{pop\_scope} libera autom\'{a}ticamente toda variable local de
          tipo con asignaci\'{o}n din\'{a}mica de memoria).
    \item \textbf{Mark-sweep c\'{i}clico}: el dise\~{n}o especifica que, para
          manejar referencias circulares, el GC debe usar una \textit{shadow
          stack} de ra\'{i}ces y que, cuando la memoria supera un umbral,
          una funci\'{o}n \texttt{hulk\_rt\_gc\_collect()} deber\'{i}a recorrer
          la lista de asignaciones (enhebrada por el campo \texttt{next} de
          \texttt{ObjHeader}) y liberar los objetos no alcanzables. Esta
          mitad del modelo \textbf{no est\'{a} implementada todav\'{i}a}: ni
          \texttt{hulk\_rt\_gc\_collect} ni el mecanismo de pila de sombra
          existen hoy en \texttt{hulk-rt}, y \texttt{hulk-codegen} tampoco
          emite a\'{u}n el mapa de campos de recolecci\'{o}n junto a cada
          tabla virtual que ese recorrido necesitar\'{i}a. La consecuencia
          directa es que, en el estado actual del compilador, todo ciclo de
          referencias que un programa HULK construya se filtra de manera
          permanente; se trata de una brecha de correcci\'{o}n reconocida,
          no de una optimizaci\'{o}n diferible, y se retoma en la
          Secci\'{o}n~\ref{sec:limitaciones}.
\end{itemize}
...
214 funciones de prueba (anotadas con \texttt{\#[test]}), distribuidas según
la siguiente tabla:
...
Dentro de \texttt{hulk-codegen}, las 71 pruebas se reparten en 31 pruebas de
integración (ELF válido, en \texttt{lib.rs}) y 40 pruebas unitarias de
descenso (verificación de IR, en \texttt{lower/mod.rs}). La cobertura por
construcción del lenguaje, reconstruida a partir de los nombres de las propias
funciones de prueba, abarca de manera explícita: literales y variables;
\texttt{let} y sombreado de variables; bloques; operadores unarios y binarios
(incluida la concatenación con y sin espacio); \texttt{if}/\texttt{elif}/
\texttt{else}; \texttt{while}; asignación destructiva a variables y a
miembros; llamadas a funciones libres con y sin argumentos, incluida
recursión; construcción de objetos (\texttt{new}); lectura de atributos;
llamadas a métodos, incluida herencia, sobreescritura y llamadas a
\texttt{base}; referencias a métodos sin invocar (tipos función); \texttt{is}/
\texttt{as}; bucles \texttt{for} sobre vectores literales y sobre
\texttt{range}; comprensiones de vectores; \texttt{match} con patrones
literales, de tipo y de cadena, así como el caso no exhaustivo; despacho a
través de protocolos; y las funciones matemáticas y constantes incorporadas.
Esta cobertura coincide, prácticamente uno a uno, con la lista de
funcionalidades -- incluidas las tres extensiones descritas en la
Sección~\ref{sec:extensiones} -- que la guía de implementación del backend
exige completar en sus fases de trabajo, lo cual sugiere que la suite de
pruebas se diseñó como un mapa de verificación directo de ese plan, y no de
manera incidental.
...
Sin embargo, el sistema fue verificado contra las pruebas provistas por los evaluadores en \href{https:/github.com/matcom/compilers}{https:/github.com/matcom/compilers/}.
...
\begin{enumerate}[leftmargin=1.4em, itemsep=0.35em]
	\item \textbf{Modelo de gestión de memoria híbrido.} El sistema actual propone combinar conteo de referencias con un recolector de trazado (mark‑sweep) para romper ciclos. Si bien esta estrategia es ampliamente utilizada en lenguajes como Python~\cite{baconrajan2004,baconrajan2001}, la implementación del recolector de trazado no está completa en la versión actual. Esto implica que los programas que construyen estructuras cíclicas pueden presentar fugas de memoria. Una evolución natural sería completar el recolector de trazado y optimizar su integración con el conteo de referencias, siguiendo las mejores prácticas de la literatura.
	...
	\item \textbf{Optimización del código generado.} El backend actual emite LLVM IR sin aplicar optimizaciones, lo que da lugar a binarios que, aunque correctos, no son tan eficientes como podrían ser. La conexión de la canalización de optimización de LLVM (pases como \texttt{mem2reg}, \texttt{instcombine} y \texttt{simplifycfg}) mejoraría notablemente el rendimiento y reduciría el tamaño del código. Esta tarea es relativamente sencilla y tendría un alto impacto.
...
\begin{enumerate}[leftmargin=1.4em, itemsep=0.35em]
	\item \textbf{Ampliación del sistema de tipos.} La introducción de tipos algebraicos (sum types) y patrones de desestructuración más ricos permitiría a HULK acercarse a lenguajes funcionales modernos. La extensión \texttt{match} ya da los primeros pasos en esa dirección; podría completarse con patrones anidados y guardas.
	
	\item \textbf{Soporte para compilación incremental y caché.} La arquitectura actual compila todo el programa en una sola pasada. Un sistema de compilación incremental, que recompile solo las partes modificadas, sería beneficioso para proyectos grandes y mejoraría la experiencia de desarrollo.
	
	\item \textbf{Generación de código para otras plataformas.} Aunque el backend está orientado a Linux x86\_64, la portabilidad de LLVM permitiría generar código para Windows, macOS y arquitecturas ARM con relativo esfuerzo. Esto haría que HULK fuera utilizable en entornos más diversos.
	
	\item \textbf{Mejora de la experiencia de usuario.} El compilador actual produce mensajes de error claros y con ubicación, pero se podría enriquecer con sugerencias automáticas (por ejemplo, corrección de errores tipográficos en nombres de variables) y una integración más estrecha con entornos de desarrollo (LSP, extensiones para editores).
\end{enumerate}
